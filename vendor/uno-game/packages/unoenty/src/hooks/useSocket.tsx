...
const useSocket = (): UseSocketResponse => {
	...
	const onGameStart = (fn: () => void) => {
		return SocketService.on<GameStartedEventData>("GameStarted", ({ game }) => {
			socketStore.setGameData(game)
	...
	const onNewChatMessage = (fn?: () => void) => {
		return SocketService.on<NewMessageEventData>("NewMessage", ({ chatId, message }) => {
	...
	const onGameListUpdated = (fn: () => void) => {
		return SocketService.on<unknown>("GameListUpdated", fn)
	}

	const onPlayerStateChange = (fn: (playerState: PlayerState, playerId: string, amountToBuy?: number) => void) => {
		const unsubBuy = SocketService.on<PlayerBuyCardsEventData>("PlayerBuyCards", ({ playerId, amountToBuy }) => {
			fn("BuyCards", playerId, amountToBuy)
		})

		const unsubBlocked = SocketService.on<PlayerBlockedEventData>("PlayerBlocked", ({ playerId }) => {
			fn("Blocked", playerId)
		})

		const unsubUno = SocketService.on<PlayerUnoEventData>("PlayerUno", ({ playerId }) => {
			fn("Uno", playerId)
		})
	...
	const onReconnect = (fn: () => void) => {
		return SocketService.on<unknown>("reconnect", async () => {
