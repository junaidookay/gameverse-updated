...
class GameService {
	private async postJson (urlString: string, headers: Record<string, string>, body: unknown): Promise<void> {
	...
	private async recordUnoClassicResults (game: Game, winnerPlayerId: string): Promise<void> {
		const baseUrl = process.env.GAMEVERSE_BASE_URL
		const internalKey = process.env.UNO_CLASSIC_INTERNAL_KEY
		...
		await Promise.all(
			players.map(async (player) => {
				const outcome = player.id === winnerPlayerId ? "win" : "loss"
				const pointsEarned = outcome === "win" ? pointsForWin : 0
				await this.postJson(
					`${baseUrl.replace(/\/+$/, "")}/api/uno`,
					{ "x-uno-internal-key": internalKey },
					{
						userId: player.id,
						outcome,
						pointsEarned,
	...
	async toggleReady (playerId: string, gameId: string): Promise<void> {
		...
		const hasEnoughPlayersToStart = game?.players?.length >= 2

		if (hasEnoughPlayersToStart && areAllPlayersReady) {
	...
	private async endGame (gameId: string): Promise<void> {
		...
		await this.recordUnoClassicResults(game, winnerInfo.id)
