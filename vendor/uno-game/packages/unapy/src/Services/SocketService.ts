...
class SocketService {
	...
	async countRoomSockets (context: SocketContext, id: string): Promise<number> {
		const roomName = this.mountRoomName(context, id)
		return await new Promise<number>((resolve) => {
			this.io.in(roomName).clients((error, clients: string[]) => {
				if (error) return resolve(0)
				resolve(Array.isArray(clients) ? clients.length : 0)
			})
		})
	}
